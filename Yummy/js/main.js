$(document).ready((_) => {
  ...
  $("#nav_Menu")
    ...
    .on("click", function () {
      ...
      if (secID == "categories") {
        // if we open categories section => get all categorise data
        searchCategories();
      } else if (secID == "area") {
        // if we open Area section => get all categorise data
        searchAreaMeals();
  ...
  async function searchMeals() {
    ...
    //     |=> if input has attr id == to searchByName Search By Name, else by First Letter
    let searchType = $(this).attr("id") == "searchByName" ? "s" : "f";

    // if We Got Empty Text => Send API with empty search
    //    rather than concate = before search text to vaildate api
  ...
  function vaildateForm() {
    ...
    // Enable Btn Sumbit If All Data are Correct
    if (validForm) {
  ...
  // check validate of User Name Input
  $("#userName").on("input", function () {
    toggleClasses(!/[^a-zA-Z\s_]/.test(this.value), this);

    // check vaildate of all input to determined enable submit input
    vaildateForm();
  });
  ...
  // check validate of User Phone Input
  ...
  // check validate of User Passowr Input
  ...
  // check validate of User Confirm Passowr Input
...
async function showMealDetails(id) {
  ...
  if (mealINFO[`strIngredient1`] != "") {
    let recipesContent = "",
      i = 1;
    while (mealINFO[`strIngredient${i}`]) { // where is there data get them
      recipesContent += `
    ...
    $("#meal-details #mealRecipes").html(recipesContent); // append Meals Recipes
  } else {
  ...
  $("#meal-details #btnSource").attr("href", strSource); // set source href
  $("#meal-details #btnYoutube").attr("href", strSource); // set youtube href

  // Show meal-details Section After Set Data
...
// Clicking on One Category => Show All Meals Of This Cat
...
// Clicking on One City => Show All Meals Of This City
...
// Clicking on One Ingredients => Show All Meals Of This Ingredients
